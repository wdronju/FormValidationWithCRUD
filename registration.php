...
if (isset($_POST['submit'])) {
    ...
    if (empty($error_name) && empty($error_Email) && empty($error_phone) && empty($errorGender) && empty($error_skill) && empty($error_country) && empty($errorFile)) {
        $sql = "INSERT INTO `users` (`name`,`email`,`phone`,`gender`,`skills`,`country`,`image`) VALUES ('$correctName','$correctEmail','$correct_phone','$correct_gender','$all_skills','$correct_country','$fileNewName')";
        ...
        if (!$result) {
            ...
            <strong>Alhamdulillah!</strong> Data Inserted Successfully
        ...
        <div class="m-auto shadow p-5 col-md-8">
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group form-floating mb-4">
                    <input class="form-control <?= isset($error_name) ? 'is-invalid' : null; ?> <?= isset($correctName) ? 'is-valid' : null; ?>" type="text" id="name" name='name' placeholder="" value='<?= $name ?? null; ?>'>
                ...
                <div class="form-group form-floating mb-4">
                    <input class="form-control <?= isset($error_Email) ? 'is-invalid' : null; ?> <?= isset($correctEmail) ? 'is-valid' : null; ?>" type="text" id="email" name='email' placeholder="" value='<?= $email ?? null; ?>'>
                ...
                <div class="form-group form-floating mb-4">
                    <input type="tel" name="phone" id="phone" placeholder="" class="form-control <?= isset($error_phone) ? 'is-invalid' : null; ?> <?= isset($correct_phone) ? 'is-valid' : null; ?>" value="<?= $phone ?? null; ?>">
                ...
                <div class="mt-4 p-3 border rounded <?= isset($errorGender) ? 'border-danger' : null; ?> <?= isset($correct_gender) ? 'border-success' : null; ?> ">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="gender" id="male" value="male" <?= isset($gender) && $gender == "male" ? "checked" : null; ?>>
                        <label class="form-check-label" for="male">Male</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="gender" id="female" value="female" <?= isset($gender) && $gender == "female" ? "checked" : null; ?>>
                ...
                <div class="mt-4 p-3 border rounded <?= isset($error_skill) ? "border-danger" : null; ?> <?= isset($correct_skills) ? "border-success" : null; ?> ">
                    ...
                    <div class="form-check form-check-inline ">
                        <input type="checkbox" name="skills[]" value="HTML" id="html" <?= isset($correct_skills) && in_array("HTML", $correct_skills) ? "checked" : null; ?>>
                        <label for="html" class="form-check-label">HTML</label>
                    </div>

                    <div class="form-check-inline ">
                        <input type="checkbox" name="skills[]" value="CSS" id="css" <?= isset($correct_skills) && in_array("CSS", $correct_skills) ? "checked" : null; ?>>
                        <label for="css" class="form-check-label">CSS</label>
                    </div>

                    <div class=" form-check-inline ">
                        <input type="checkbox" name="skills[]" value="JS" id="js" <?= isset($correct_skills) && in_array("JS", $correct_skills) ? "checked" : null; ?>>
                        <label for="js" class="form-check-label">JS</label>
                    </div>

                    <div class=" form-check-inline ">
                        <input type="checkbox" name="skills[]" value="PHP" id="php" <?= isset($correct_skills) && in_array("PHP", $correct_skills) ? "checked" : null; ?>>
                ...
                <select class="form-select py-3 <?= isset($correct_country) ? 'border-success' : null; ?> <?= isset($error_country) ? 'border-danger' : null; ?>" aria-label="Default select example" name='country'>
                    ...
                    <option value="Bangladesh" <?= isset($country) && $country == 'Bangladesh' ? 'selected' : null; ?>>Bangladesh</option>
                    <option value="India" <?= isset($country) && $country == 'India' ? 'selected' : null; ?>>India</option>
                    <option value="Pakistan" <?= isset($country) && $country == 'Pakistan' ? 'selected' : null; ?>>Pakistan</option>
                ...
                <input type="submit" value="Submit" class="btn btn-lg btn-success mt-3" name='submit'>
            </form>
        </div>
